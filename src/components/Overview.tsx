...
/* confusion matrix, feature importance, tenure EDA and the training facts      */
/* ============================================================================ */
export function Overview({ model, threshold, onThreshold }: { model: TrainedModel; threshold: number; onThreshold: (t: number) => void }) {
  const testLabels = useMemo(() => model.testIndex.map((i) => (model.customers[i].churned ? 1 : 0)), [model]);
  const testScores = useMemo(() => model.testIndex.map((i) => model.probabilities[i]), [model]);
  const cm = useMemo(() => confusionAt(testLabels, testScores, threshold), [testLabels, testScores, threshold]);

  const importanceRows = useMemo(() => {
    const rows = model.importance.map((value, i) => ({ name: model.matrix.names[i], value }));
    rows.sort((a, b) => b.value - a.value);
    return rows.filter((r) => r.value > 0.004).slice(0, 9);
  }, [model]);

  const churnedCount = model.customers.filter((c) => c.churned).length;
  const kept = model.customers.length - churnedCount;
  const imbalance = kept / Math.max(1, churnedCount);

  return (
    <div className="anim-fade-up space-y-4">
      <div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-5">
        <MetricTile label="ROC · AUC" value={model.curves.rocAuc} color="#33d6ae" note="Ranking quality across every threshold" />
        <MetricTile label="PR · AUC" value={model.curves.prAuc} color="#5fa8f2" note="Imbalance-aware precision–recall area" />
      ...
      <div className="grid gap-3 lg:grid-cols-3">
        <div className="panel p-4">
          ...
          <CurveChart
            ...
            metric="AUC"
            metricValue={model.curves.rocAuc}
          />
        </div>
        <div className="panel p-4">
          <SectionHead title="Precision–Recall" sub="The honest view under class imbalance" />
          <CurveChart
            points={model.curves.pr}
            color="#5fa8f2"
            areaFrom="curve"
            xLabel="recall"
            yLabel="precision"
            metric="PR-AUC"
            metricValue={model.curves.prAuc}
          />
        </div>
        <div className="panel p-4">
        ...
        <div className="panel flex flex-col p-4">
          ...
          <dl className="grid flex-1 grid-cols-2 content-start gap-x-4 gap-y-3">
            {[
              ...
              ["Trees", `${model.gbdt.trees.length} × depth 4`],
              ["Learning rate", "0.085"],
              ["Subsampling", "85% per tree"],
              ["L2 regularizer", "λ = 1.2"],
              ["Train rows", `${model.trainIndex.length.toLocaleString()}`],
              ["Test rows", `${model.testIndex.length.toLocaleString()}`],
              ["Fit time", `${(model.trainedMs / 1000).toFixed(2)} s`],
              ["Random seed", `#${model.seed}`],
              ["Churn rate", fmtPct(model.churnRate)],
              ...
              <div key={k} className="rounded-md border border-linesoft bg-canvas/50 px-2.5 py-2">
                ...
                <dd className="mt-0.5 truncate font-mono text-[12px] font-medium text-ink" title={v}>
                  {v}
                </dd>
              </div>
            ))}
          ...
          <div className="mt-3 flex items-center gap-2 rounded-md border border-teal/25 bg-teal/8 px-3 py-2 text-[11px] text-teal">
            <IconPulse size={14} />
            Model re-fits live in your browser — no server round trip.
          </div>
        </div>
      </div>
    </div>
...
+++ src/components/Overview.tsx (修改后)
import { useMemo } from "react";
